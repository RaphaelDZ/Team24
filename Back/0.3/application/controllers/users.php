...
class Users extends CI_Controller
{
    ...
    function login()
    {
        if($this->session->userdata("user_id"))//Si déjà conncecté
        {
            redirect(base_url());//redirection vers la page blog
        }
        $data['error'] = 0;
        if($this->input->post())
        {
            ...
            if(!$user){ $data['error'] = 1;}//quand utilisateur n'exite pas
            else //quand utilisateur existe
            {
